...
class SchemaValidator:
    ...
    def validate_schema_compatibility(
        ...
        feature_columns: list[str] = (
            [col for col in new_data.columns if col != target_column] 
            if target_column 
            else list(new_data.columns)
        )

        if "n_features" in existing_schema and len(feature_columns) != existing_schema["n_features"]:
            errors.append(
                f"Feature count mismatch: {len(feature_columns)} vs {existing_schema['n_features']}"
            )
        ...
        if "feature_names" in existing_schema:
            ...
            if feature_columns != expected_names:
                errors.append(
                    f"Feature names/order mismatch: {feature_columns} vs {expected_names}"
                )
            ...
            for feature_def in existing_schema["structural_schema"]:
                ...
                if col_name in new_data.columns:
                    ...
                    if actual_dtype != expected_dtype:
                        errors.append(
                            f"Data type mismatch for '{col_name}': {actual_dtype} vs {expected_dtype}"
                        )
